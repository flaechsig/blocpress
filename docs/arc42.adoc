...
| HTTPS, Web Components (Import Maps)
...
| Jedes SCS-Modul liefert eigene UI als Web Components (Custom Elements mit Shadow DOM). Portal-Shell (blocpress-studio) lädt diese via Import Maps.
...
| *blocpress-studio*
| Portal-Shell: Navigation, Authentifizierung (JWT), Integration der Micro-Frontends via Import Maps. Kein eigenes Backend.
| —
...
* Studio lädt die Web Components über Import Maps (z.B. `blocpress-workbench-ui.js`)
...
| blocpress-studio
| Portal-Shell: Authentifizierung (JWT-Handling), Navigation, Laden der Micro-Frontends via Import Maps. Kein eigenes Backend — rein statische HTML/JS-Anwendung.

| blocpress-workbench
| Template-Entwicklung: Upload, Validierung (TF-1), Bearbeitung, Bausteinverwaltung, Content Search, Vorschau-Generierung. Use Cases: UC-1 bis UC-5, UC-19 bis UC-21.
...
| blocpress-render
| Dokumentengenerierung: Einzige öffentliche REST-API (TI-1). Liest freigegebene Templates aus Schema `production`. LibreOffice-Integration (TF-5). Stateless, horizontal skalierbar.
...
Integration mit LibreOffice headless zur Verarbeitung von ODT-Templates. Wird in blocpress-workbench (Validierung) und blocpress-render (Generierung) eingesetzt.

*Schnittstellen:*

* `validateTemplate(byte[] odtBinary): ValidationResult` - TF-1 (in blocpress-workbench)
* `generateDocument(Template template, String jsonData, OutputFormat format): byte[]` - TF-5 (in blocpress-render)
...
services:
  ...
  blocpress-studio:
    image: nginx:alpine
    volumes:
      - ./studio/dist:/usr/share/nginx/html
...
* Studio lädt die Bundles dynamisch via Import Maps
...
{
  "imports": {
    "bp-workbench": "/modules/workbench/bp-workbench.js",
    "bp-proof": "/modules/proof/bp-proof.js",
    "bp-admin": "/modules/admin/bp-admin.js"
  }
}
</script>
----
...
LibreOffice ≥ 24 headless, integriert in blocpress-render und blocpress-workbench
...
| *blocpress-studio*
| Portal-Shell: Keine eigene Business-Logik. Verantwortlich für Navigation, JWT-Authentifizierung und Integration der Micro-Frontends (Web Components) via Import Maps.
...
| Prozess der automatisierten Befüllung eines Templates mit JSON-Daten und Export als ODT/PDF/RTF. Ausgeführt von blocpress-render. Entity E-3, TF-5.
...
| *Import Map*
| Browser-Standard zur Auflösung von JavaScript-Modulnamen auf URLs. Verwendet von blocpress-studio zum dynamischen Laden der Web Component-Bundles der SCS-Module.

| *LibreOffice headless*
| LibreOffice-Installation ohne grafische Oberfläche, die über UNO-API programmgesteuert bedient wird. Version ≥ 24 erforderlich. Eingesetzt in blocpress-render und blocpress-workbench.
