...
| Übersichtliche Darstellung fälliger Reviews in der Web-Oberfläche. Nachvollziehbare Review-Historie.
...
Core -right-> Monitor : OTLP (gRPC/HTTP)
...
| blocpress-server → OpenTelemetry Collector
...
* Secondary Ports: DatabasePort, StoragePort, SearchPort, QueuePort
* Adapters: PostgresAdapter, ElasticsearchAdapter, RabbitMQAdapter
...
| Compliance Manager
| Überwachung von Review-Zyklen (E-5), Erstellung fälliger Review-Einträge, geplanter Job
...
note right of CM
  ...
  Fällige Reviews sind auf UI-3 sichtbar
end note
@enduml
----
...
services:
  ...
  blocpress-core:
    ...
    environment:
      ...
      # OpenTelemetry (optional — nur setzen, wenn Collector vorhanden)
...
* *Überwachbarkeit:* Liveness-/Readiness-Probes für Health-Checks, OpenTelemetry für Traces/Metriken/Logs (optional)
...
blocpress-server nutzt *Quarkus OpenTelemetry* (`quarkus-opentelemetry`) für die Erfassung von Traces, Metriken und Logs. Die Integration ist vendor-neutral und erfordert vom Betreiber lediglich die Konfiguration eines OTLP-Endpunkts.
...
quarkus.otel.resource.attributes=service.name=blocpress-server
...
Wird kein `quarkus.otel.exporter.otlp.endpoint` konfiguriert, ist OpenTelemetry deaktiviert — blocpress läuft ohne externe Monitoring-Abhängigkeit. Der Betreiber kann jeden OTLP-kompatiblen Collector einsetzen (z.B. Grafana Alloy, Jaeger, Datadog Agent).
