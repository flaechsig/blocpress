...
* xref:Element_Design_Concept.adoc#edc-goals[Element Design Concept: blocpress Core] -- Goals, Use Cases, Technical Functions, Entities
...
| Quarkus Framework
| blocpress Core muss mit Quarkus implementiert werden (schnelle Startup-Zeit, geringer Memory-Footprint, native Kompilierung möglich).

| LibreOffice ≥ 24
| LibreOffice Version 24 oder höher muss headless in blocpress Core integriert sein. Ältere Versionen nicht unterstützt.
...
| Benutzer-Authentifizierung muss über externen IdP erfolgen (SAML 2.0 oder OpenID Connect).
|===
...
System --> IdP : Benutzer authentifizieren
...
| Identity Provider
| SAML/OIDC Token
| Authentifizierungsergebnis, Benutzerattribute
...
* Horizontal skalierbar: Mehrere blocpress Core-Instanzen hinter Load Balancer
...
* *Entwicklungsprozess:* Agil (Scrum), 2-Wochen-Sprints
* *CI/CD:* Automatische Tests bei jedem Commit, automatisches Deployment zu Test-Umgebung
* *Code-Reviews:* Vier-Augen-Prinzip auch bei Code (mindestens 1 Reviewer)
...
* *blocpress Core* enthält gesamte Business-Logik
...
| API Gateway
| Authentifizierung (Integration mit Identity Provider), Rate Limiting, Request-Logging, Routing zu blocpress Core.
...
* *TI-1 (REST-API):* Primäre Schnittstelle zwischen Web-Oberfläche/externen Anwendungen und blocpress Core
...
package "blocpress Core (Quarkus)" {
  component "REST-API\nController" as API
  component "Use Case\nServices" as UC
  component "Technical\nFunctions" as TF
  component "Workflow\nEngine" as WF
...
* Package: `de.blocpress.core.libreoffice`
...
* Interface: `de.blocpress.core.storage.StorageService`
* Implementierung: `de.blocpress.core.storage.PostgresStorageService`
* Zukünftig: `de.blocpress.core.storage.S3StorageService`
...
* Integration mit externem Identity Provider via SAML 2.0 oder OpenID Connect
* Single Sign-On (SSO) für Web-Oberfläche
* API-Keys für externe Anwendungen (generiert durch Administrator)
* Token-Validierung bei jedem Request im API Gateway
...
Wahl des Java-Frameworks für blocpress Core. Optionen: Spring Boot, Quarkus, Micronaut.
...
* Risiko: Team muss Quarkus lernen (Schulung einplanen)
...
LibreOffice ≥ 24 headless, integriert in blocpress Core
...
| *Workflow-Engine*
| Komponente in blocpress Core zur Steuerung von Freigabeprozessen und Statusübergängen. Prüft Berechtigungen und Vier-Augen-Prinzip.
|===
