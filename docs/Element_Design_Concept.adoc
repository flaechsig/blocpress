= Element Design Concept: blocpress-server
...
*Description:*
blocpress-server soll LibreOffice-Templates (ODT) mit strukturierten JSON-Daten automatisiert zusammenführen und Dokumente in den Formaten ODT, PDF und RTF generieren. Die Generierung muss deterministisch, performant und über REST-API zugänglich sein. +
...
*Description:*
blocpress-server soll alle Templates, Bausteine und deren Versionen zentral in PostgreSQL speichern und verwalten. Änderungen müssen nachvollziehbar sein und der Freigabestatus jederzeit erkennbar. +
...
*Description:*
blocpress-server soll einen mehrstufigen Freigabeprozess mit definierten Rollen (Template-Designer, Qualitätsmanager, Freigeber) implementieren. Templates dürfen nur nach erfolgreicher Prüfung in Produktion verwendet werden. +
...
*Description:*
blocpress-server soll automatisch Review-Zyklen (1, 3 oder 5 Jahre) überwachen, fällige Reviews identifizieren und Compliance-Reviewer benachrichtigen. +
...
*Description:*
blocpress-server soll Testdaten verwalten, Test-PDFs generieren, mit Baselines vergleichen und Abweichungen erkennen. +
...
*Description:*
blocpress-server soll eine Integration mit Elasticsearch bereitstellen, um fachliche Konstrukte über alle Templates und Bausteine hinweg innerhalb von 2 Sekunden zu finden. +
...
5a. blocpress-server gibt Fehlermeldung zurück
...
8. blocpress-server ruft TF-3 (Benachrichtigung senden) auf
9. TF-3 sendet E-Mail über TI-6 (SMTP) an alle Benutzer mit Rolle "Qualitätsmanager"
10. blocpress-server bestätigt erfolgreiche Einreichung an Web-Oberfläche
...
11. blocpress-server ruft TF-3 (Benachrichtigung senden) auf
12. TF-3 sendet E-Mail über TI-6 (SMTP) an ursprünglichen Designer
13. blocpress-server bestätigt erfolgreiche Freigabe an Web-Oberfläche
...
6a. blocpress-server gibt Validierungsfehler zurück
...
14a. blocpress-server aktualisiert Status in E-3 auf "Fehlgeschlagen" mit Fehlerprotokoll
...
13. blocpress-server ruft TF-3 (Benachrichtigung senden) auf
14. TF-3 sendet E-Mail über TI-6 (SMTP) an Template-Designer (bei "Überarbeitung erforderlich")
15. blocpress-server bestätigt erfolgreiche Review-Speicherung an Web-Oberfläche
16. Web-Oberfläche zeigt Bestätigung auf UI-3 an

*Alternative Scenarios:*

7a. Review-Ergebnis ist "Archivieren"
8a. blocpress-server ändert Template-Status in E-1 zu "Archiviert"
...
*Description:*
Zugriff auf PostgreSQL-Datenbank zum Lesen und Schreiben aller Entities. Wird intern von blocpress-server genutzt.
...
*Description:*
Asynchrone Kommunikation für zeitaufwändige Operationen (Dokumentengenerierung, Regressionstests). Wird intern von blocpress-server genutzt.
...
*Description:*
Zugriff auf Content Search Engine zur Indexierung und Suche. Wird von blocpress-server für UC-19 (Nach Begriff suchen) genutzt.
...
*Description:*
blocpress-server muss mit Quarkus Framework implementiert werden. Dies ermöglicht schnelle Startup-Zeiten, geringen Memory-Footprint und native Kompilierung.
...
*Description:*
blocpress-server muss als Docker-Container betrieben werden können. Dockerfile und docker-compose.yml müssen bereitgestellt werden.
