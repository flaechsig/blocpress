...
*blocpress-workbench (Quarkus):* Template-Entwicklung — Upload, Validierung (TF-1), Bearbeitung, Bausteinverwaltung, Content Search (Elasticsearch), Vorschau-Generierung. Arbeitsumgebung für Template-Designer. Nutzt Schema `workbench`. +
...
*blocpress-render (Quarkus):* Dokumentengenerierung — einzige öffentliche REST-API. Liest freigegebene Templates aus Schema `production`. Integriert LibreOffice headless (Version ≥ 24). Stateless, horizontal skalierbar. +
...
6. blocpress-workbench sendet die Template-Datei an den LibreOffice Document Processor zur Validierung
7. Der Document Processor analysiert User-Fields, Wiederholungsgruppen und IF-Bedingungen und meldet Ergebnisse zurück
...
- LibreOffice Version ≥ 24 muss headless in blocpress-render und blocpress-workbench integriert sein
