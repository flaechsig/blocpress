...
*blocpress-server (Backend):* Quarkus-basierte Anwendung, die die Geschäftslogik implementiert. Verwaltet Templates, Bausteine, Versionen, Workflows, Compliance-Reviews und Tests. Stellt REST-API und interne Services bereit. Integriert LibreOffice headless (Version ≥ 24) für die Dokumentenverarbeitung. +
_Detaillierte Beschreibung: xref:Element_Design_Concept.adoc#edc-goals[Goals im Element Design Concept]_

*Web-Oberfläche (Frontend):* React-basierte Single-Page-Application für Template-Designer, Qualitätsmanager, Compliance-Reviewer, Test-Manager und Administratoren. Kommuniziert mit blocpress-server über REST-API. +
_User Interfaces: xref:Element_Design_Concept.adoc#edc-uis[User Interfaces im Element Design Concept]_

*LibreOffice Document Processor:* Integrierte Komponente in blocpress-server, die LibreOffice headless (Version ≥ 24) zur Verarbeitung von Templates nutzt. Befüllt User-Fields in Punkt-Notation, führt Wiederholungsgruppen aus, wertet IF-Bedingungen aus und exportiert Dokumente als ODT, PDF oder RTF. +
...
*Workflow Engine:* Komponente innerhalb von blocpress-server, die Freigabeprozesse und Compliance-Reviews steuert. Verwaltet Zustände, Übergänge und Berechtigungsprüfungen. +
...
*Test Framework:* Komponente innerhalb von blocpress-server, die Testdaten verwaltet, PDF-Vergleiche durchführt und Regressionstests automatisiert. Speichert Baselines und Test-Ergebnisse. +
_Technical Function: xref:Element_Design_Concept.adoc#edc-tf-6[TF-6: PDF vergleichen]_

*Compliance Manager:* Komponente innerhalb von blocpress-server, die Review-Zyklen überwacht, Fälligkeitstermine berechnet und fällige Review-Einträge erstellt. Compliance-Reviewer sehen fällige Reviews auf der Web-Oberfläche (UI-3). +
...
*Load Balancer:* Hardware- oder Software-Load-Balancer zur Verteilung von Last auf mehrere blocpress-server-Instanzen (optional, für Hochverfügbarkeit).
...
*JWT-basierte Authentifizierung:* blocpress-server validiert eingehende JSON Web Tokens (JWT) direkt mithilfe eines konfigurierten Issuers und Public Keys (Quarkus SmallRye JWT). Es besteht keine Laufzeitabhängigkeit zu einem externen Identity Provider. +
_Technical Interface: xref:Element_Design_Concept.adoc#edc-ti-4[TI-4: JWT-Validierung]_

*Observability (optional):* blocpress-server exportiert Traces, Metriken und Logs über OpenTelemetry (OTLP-Protokoll). Der Betreiber kann einen beliebigen OTLP-kompatiblen Collector einsetzen (z.B. Grafana Alloy, Jaeger, Datadog Agent). Ohne konfiguriertem Endpoint läuft das System ohne externe Monitoring-Abhängigkeit.
...
1. Der Template-Designer meldet sich über die Web-Oberfläche an (Authentifizierung erfolgt über JWT)
2. Die Web-Oberfläche sendet eine Anfrage an blocpress-server zur Verifizierung der Benutzerrolle
3. blocpress-server prüft die Berechtigungen und gibt die Template-Verwaltungsseite frei
4. Der Template-Designer lädt eine LibreOffice-Datei (.odt) über die Web-Oberfläche hoch
5. Die Web-Oberfläche sendet die Datei an blocpress-server über die REST-API
6. blocpress-server speichert die Datei als Binärdaten im Template Repository (PostgreSQL) und erstellt einen Metadaten-Eintrag
7. blocpress-server sendet die Template-Datei an den LibreOffice Document Processor zur Validierung
...
9. blocpress-server speichert die Validierungsergebnisse und aktualisiert den Template-Status auf "Entwurf"
10. Der Template-Designer sieht die Validierungsergebnisse in der Web-Oberfläche und reicht das Template zur Freigabe ein
11. Die Workflow Engine ändert den Status auf "Eingereicht" — eingereichte Templates sind für Qualitätsmanager auf UI-2 sichtbar
...
2. Das API Gateway authentifiziert die Anfrage und leitet sie an blocpress-server weiter
3. blocpress-server validiert die JSON-Daten gegen das Template-Schema
4. blocpress-server lädt das Template aus dem Template Repository (PostgreSQL)
5. blocpress-server legt die Generierungsanfrage in die Message Queue für asynchrone Verarbeitung
...
10. blocpress-server aktualisiert den Generierungs-Status auf "Erfolgreich" und speichert die Metadaten im Template Repository
11. Die externe Anwendung erhält die Generierungs-ID als Response und kann den Status abfragen
12. Beim Status-Abruf liefert blocpress-server einen Download-Link für das generierte PDF zurück
...
10. Der Test-Manager öffnet die Web-Oberfläche und sieht die Test-Ergebnisse auf UI-4
11. Der Test-Manager entscheidet, ob Abweichungen akzeptabel sind oder das Template überarbeitet werden muss
...
5. Der Compliance-Reviewer meldet sich in der Web-Oberfläche an und sieht fällige Reviews auf UI-3
6. Der Reviewer öffnet ein Template zur Prüfung und vergleicht es mit aktuellen rechtlichen und fachlichen Anforderungen
7. Der Reviewer hinterlässt Kommentare im System und markiert das Template als "Freigegeben" oder "Überarbeitung erforderlich"
8. blocpress-server aktualisiert den Review-Status und berechnet das nächste Review-Datum
9. Bei "Überarbeitung erforderlich" ist das Template auf UI-1 als "Überarbeitung erforderlich" markiert
...
3. Die Web-Oberfläche sendet eine Such-Anfrage an blocpress-server über die REST-API
4. blocpress-server leitet die Anfrage an die Content Search Engine (Elasticsearch) weiter
...
7. blocpress-server reichert die Suchergebnisse mit Metadaten (Template-Name, Status, Version) aus dem Template Repository an
...
2. blocpress-server speichert das Template als Binärdaten im Template Repository (PostgreSQL) und aktualisiert die Metadaten
3. blocpress-server sendet eine Indexierungs-Anfrage an die Content Search Engine
...
7. Die Search Engine bestätigt die erfolgreiche Indexierung an blocpress-server
...
*Skalierbarkeit:* Das System soll bis zu 100 gleichzeitige Dokumentengenerierungen verarbeiten können. Die Architektur soll horizontal skalierbar sein, sodass bei Bedarf weitere blocpress-server-Instanzen hinzugefügt werden können.
...
*Sicherheit - Authentifizierung:* Der Zugriff auf die Web-Oberfläche und die REST-API muss durch Authentifizierung geschützt sein. blocpress-server validiert JWTs direkt mittels konfiguriertem Issuer und Public Key (Quarkus SmallRye JWT). Eine Laufzeitabhängigkeit zu einem externen Identity Provider besteht nicht.
...
*Observability:* blocpress-server unterstützt OpenTelemetry (OTLP) für den Export von Traces, Metriken und Logs. Die Konfiguration eines OTLP-Endpoints ist optional — ohne Endpoint läuft das System ohne externe Monitoring-Abhängigkeit. Quarkus Health-Checks (`/q/health/live`, `/q/health/ready`) stehen unabhängig davon zur Verfügung.
...
- LibreOffice Version ≥ 24 muss headless in blocpress-server integriert sein
