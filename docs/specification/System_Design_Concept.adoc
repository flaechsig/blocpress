...
*blocpress-workbench (Quarkus):* Template-Entwicklung — Upload, Validierung (TF-1 via blocpress-core), Bearbeitung, Bausteinverwaltung (Entity E-2, inkl. WebDAV-Zugriff), Content Search (Elasticsearch), Vorschau-Generierung, **Template Management Dashboard mit Status-Filtern und kombinierter Statusansicht** (UC-5: Status-Übergänge, Duplikation, Re-Upload). **WebDAV-Endpoint** (`/api/webdav/`) für direkten Baustein- und Template-Zugriff aus LibreOffice (DAV Level 1: GET/PUT/PROPFIND; DRAFT read-write, APPROVED read-only unter `/released/`). Arbeitsumgebung für Template-Designer. Nutzt Schema `workbench`. +
...
*PostgreSQL-Datenbank (Version ≥ 18):* Zwei PostgreSQL-Instanzen: `workbench` (Entwicklung und Prüfung, Schema für blocpress-workbench) und `production` (nur freigegebene Templates, Schema für blocpress-render). Freigegebene Templates werden bei APPROVED physisch in die production-Datenbank kopiert (REST-basierter Deploy via `POST /api/render/templates/import`). Binärdaten als bytea. +
...
*Datenbank-Server:* Dedizierter Server oder Container für PostgreSQL (Version ≥ 18) mit persistentem Storage. Zwei Datenbankinstanzen: `workbench` (für blocpress-workbench) und `production` (für blocpress-render). Mindestens 8 GB RAM, 100 GB SSD-Storage.
...
*Ziel:* Ein Template-Designer verwaltet seine Templates über ein visuelles Dashboard mit Status-Filtern und führt Workflow-Übergänge durch.

1. Der Template-Designer meldet sich über blocpress-studio an
2. blocpress-studio lädt das Web Component `<bp-workbench>` und reicht das JWT als Property weiter
3. Der Template-Designer öffnet blocpress-workbench und sieht das Template Management Dashboard (statt Autocomplete-Suche)
4. Das Dashboard zeigt alle Templates als Karten in Gitteransicht mit Status-Badges (DRAFT, SUBMITTED, APPROVED, REJECTED)
5. Der Template-Designer filtert Templates nach Status (z.B. "🧪 Test" zeigt nur SUBMITTED-Templates)
6. Für DRAFT-Templates sieht der Designer Aktionen: Öffnen, Aktualisieren, → Test, Löschen
7. Der Designer klickt "→ Test" um ein DRAFT-Template zur Freigabe einzureichen (Status ändert sich zu SUBMITTED)
8. blocpress-workbench validiert die Statusübergangs-Logik und speichert den neuen Status
9. Für APPROVED-Templates sieht der Designer "Als Kopie" Button
10. Der Designer klickt "Als Kopie", gibt einen Namen ein, und ein neues DRAFT-Template wird erstellt (Basis für neue Entwicklung)
11. Für DRAFT-Templates sieht der Designer "Aktualisieren" Button zum Re-Upload einer aktualisierten ODT-Datei
12. Das Template wird neu validiert und die Validierungsergebnisse aktualisiert
13. Der Designer klickt auf ein Template-Card "Öffnen" um den Workspace mit Testdaten und Vorschau zu öffnen
14. Der Designer kann über "← Zurück zum Dashboard" zurück zur Übersicht gehen

_Use Case: xref:Element_Design_Concept.adoc#edc-uc-5[UC-5: Template Management Dashboard mit Status-Filtern und Workflow]_ +
_Technical Functions: Status-Übergänge mit Validierung (von TF-2), Template-Duplikation, Re-Upload (TF-1 Neu-Validierung)_
...
5. Der Document Processor befüllt das Template mit den JSON-Daten (User-Fields, Wiederholungsgruppen, IF-Bedingungen) synchron im selben Request
6. Der Document Processor exportiert das befüllte Dokument im gewünschten Format (ODT, PDF oder RTF)
7. blocpress-render liefert das generierte Dokument direkt als Response zurück
...
1. **Integration Tests mit realen Abhängigkeiten:** H2 in-memory Datenbanken für Test-Isolation
2. **Echtheit vor Mocks:** Tests durchlaufen die komplette Schicht (HTTP → REST → Business Logic → Database)
3. **Bug-Discovery by Scenario:** Echte Datenbank-Szenarien decken Fehler auf, die Mock-Tests übersehen
4. **Determinismus-Validierung:** Expected Result PDFs verifizieren Qualitätsziel "Zuverlässigkeit"
...
- JaCoCo-Coverage für REST-APIs ist nicht aussagekräftig (Code läuft in separatem ClassLoader, HTTP-Marshalling wird nicht instrumentiert)
