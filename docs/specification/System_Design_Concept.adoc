...
*blocpress-workbench (Quarkus):* Template-Entwicklung — Upload, Validierung (TF-1 via blocpress-core), Bearbeitung, Bausteinverwaltung, Content Search (Elasticsearch), Vorschau-Generierung, **Template Management Dashboard mit Status-Filtern und Workflow** (UC-5: Status-Übergänge, Duplikation, Re-Upload). Arbeitsumgebung für Template-Designer. Nutzt Schema `workbench`. +
