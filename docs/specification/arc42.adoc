...
=== ADR-003: Synchrone Dokumentengenerierung
...
Synchrone Verarbeitung: API-Call wartet bis das Dokument fertig generiert ist und gibt das Ergebnis direkt zurück.

*Begründung:*

* *Einfachheit:* Kein asynchroner Mechanismus (Queue, Polling, Status-Endpunkt) notwendig
* *Zuverlässigkeit:* Direkte Fehlerbehandlung möglich
* *Ausreichend:* Für die aktuelle Last (Workbench-Vorschau, produktive Einzelgenerierung) ist synchrones Rendering schnell genug

*Konsequenzen:*

* Positiv: Einfachere Architektur, direktes Feedback
* Negativ: Bei sehr langen Generierungen (> 60s) können HTTP-Timeouts auftreten
* Mitigation: Timeout auf 60 Sekunden konfiguriert; für Batch-Szenarien kann später ergänzt werden
