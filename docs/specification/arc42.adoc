...
WB -> RD : POST /api/render/templates/import\n{id, name, version, contentBase64, validFrom}
...
note over RD
  ...
  (render liest aus production-DB,
   nicht mehr von workbench)
end note
...
NOTE: Die Template-Tabelle existiert mit identischer Struktur in den Datenbanken `workbench` und `production`. TI-2 (Option A) ist implementiert: Direktes physisches Kopieren via REST-API (`POST /api/render/templates/import`). Die Schemata `proof` und `admin` sind für zukünftige Module geplant.
