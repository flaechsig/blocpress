...
* *Workflow-gesteuerte Freigabe:* Mehrstufiger Freigabeprozess mit Reviewer-Genehmigung (organisatorisch durchgesetzt)
* *Compliance-Management:* Automatische Überwachung turnusmäßiger Reviews (1, 3 oder 5 Jahre)
* *Test-Framework:* Verwaltung von Testdaten, Baseline-PDFs, automatisierte Regressionstests mit Inline-Diff und Coverage-Analyse (Felder, Bedingungen, Wiederholungsgruppen)
...
| 3
| *Wartbarkeit*
| Templates und Code müssen versioniert sein. Änderungen nachvollziehbar.
| 9, 10.3

| 4
| *Sicherheit*
| Reviewer-Freigabe (organisatorisch durchgesetzt). RBAC für alle Funktionen. API-Rate-Limiting.
| 2.3, 10.4

| 5
| *Skalierbarkeit*
| Horizontal skalierbar: Mehrere parallele blocpress-render-Instanzen hinter einem Load Balancer.
| 8.1, 10.5
|===
...
| Reviewer-Freigabe erforderlich
| Templates müssen von Reviewer-Rolle freigegeben werden. Organisatorisch sicherzustellen, dass nicht der Designer sein Template selbst freigibt.
...
| blocpress-workbench → blocpress-render
| HTTPS, REST-API, JSON (intern)
| Template-Deploy: `POST /api/render/templates/import` — automatisch bei APPROVED-Status, kopiert Template in production-Schema

| SCS-Module → PostgreSQL
| JDBC, PostgreSQL Wire Protocol
| blocpress-workbench nutzt Schema `workbench`, blocpress-render nutzt Schema `production`
...
| Reife relationale DB mit zwei Schemata: `workbench` für Entwicklung/Test und `production` für freigegebene Templates. Identische Tabellenstrukturen ermöglichen physische, kopie-basierte Stufenübergabe — nur freigegebene Templates werden nach production kopiert, um nachzuweisen, dass nur validierte Versionen in Produktion verwendet werden.
| Zuverlässigkeit, Wartbarkeit, Compliance

| *Elasticsearch 8.11 für Content Search*
| Hochperformante Volltextsuche via `quarkus-elasticsearch-rest-client`. Index `blocpress-templates` mit german Analyzer für Freitext, keyword-Felder für Filter. `OdtTextExtractor` (blocpress-core) extrahiert Plaintext aus ODT-Bytes. Indexierung best-effort bei Upload/Status-Änderung/Löschung. Im Quickstart-Image als supervisord-Prozess, in docker-compose als eigener Service.
| Performance (< 2s Suchzeit), UC-19
...
| Alle Quarkus-Module (`render`, `workbench`, `studio`) sind via `native`-Profil als eigenständige Native-Binaries baubar (Mandrel-Container-Build, kein lokales GraalVM). Startzeit ~0,06 s, kein JRE im Image, geringerer Speicher-Footprint. odfdom/Xerces benötigen Reflection-Konfiguration (in `blocpress-core`). `blocpress-render` nutzt im Native-Image den LibreOffice-CLI-Pfad statt JODConverter (dessen UNO-Jars sind versiegelt und nicht native-kompatibel); Nebenläufigkeit wird über ein Semaphore-Throttling mit Profil-Isolation pro `soffice`-Aufruf abgebildet.
| Performance (Startzeit, Memory), Betriebseffizienz
|===
...
| *blocpress-workbench*
| Template-Entwicklung: Upload, Validierung, Bearbeitung, Bausteinverwaltung, Content Search, Vorschau-Generierung, **Dashboard mit Status-Filtern und Workflow** (UC-5). Arbeitsumgebung für Template-Designer.
| Schema `workbench`
...
* *CI/CD:* GitHub Actions — CI bei jedem Commit/PR, vollautomatischer Release per `mvn validate -Ptrigger-release` (workflow_dispatch: Version bump, Tests, Maven Central, Docker Hub, GitHub Release, Branch-Merge)
...
package "blocpress Gesamtsystem" {
  ...
  component "blocpress-workbench\n(Quarkus)" as Workbench
  component "blocpress-proof\n(Quarkus) [geplant]" as Proof
  component "blocpress-render\n(Quarkus)" as Render
  component "blocpress-admin\n(Quarkus) [geplant]" as Admin

  component "blocpress-core\n(Shared Library)" as Core

  database "PostgreSQL\n(workbench | production)" as DB
...
* *PostgreSQL* nutzt getrennte Datenbanken für workbench und production — identische Template-Tabellenstrukturen ermöglichen kopie-basierte Stufenübergabe via REST
...
| blocpress-studio
| Portal-Shell (Quarkus): JWT-Handling (Token-Guard, LocalStorage-basiert für Quickstart), Navigation, Laden von `bp-workbench.js` via Dynamic Import über internen Proxy-Endpoint (`/proxy/bp-workbench.js`). Im Quickstart-Modus: transparenter API-Proxy für alle `/api/*`-Browser-Requests an blocpress-workbench (intern). Liefert statische HTML/JS-Assets mit deaktiviertem Browser-Caching aus.
...
| Persistente Speicherung mit zwei Datenbanken: `workbench` (Entwicklung/Test) und `production` (freigegebene Templates). Identische Template-Tabellenstruktur in beiden Datenbanken. Übergabe via REST-API (TI-2 Option A).
...
API -down-> WebDAV : aufrufen
TS -down-> Core : TF-1\n(Validierung)
TS -down-> Storage : save/load
TS -down-> Repo : CRUD
BS -down-> Repo : CRUD
WebDAV -down-> Repo : CRUD
...
package "blocpress-render (Quarkus)" {
  component "RenderResource\n(sync, öffentlich)" as API
  component "AsyncRenderResource\n(UC-23: /api/render/jobs/*)" as AsyncAPI
  component "RenderDashboardResource\n(/api/render/dashboard)" as Dashboard
  component "RenderJobWorker\n(@Scheduled poll 2s\n@Scheduled cleanup 1h)" as Worker
  component "TemplateCache\n(Caffeine, 10min TTL)" as Cache
  component "LibreOffice\nPool" as LO
  component "WebhookSender\n(VirtualThread)" as Webhook
  component "Repository\nLayer" as Repo
}

component "blocpress-core\n(Shared Library)" as Core

API -down-> Cache : getTemplateContent
API -down-> Core : mergeTemplate
API -down-> LO : convert

AsyncAPI -down-> Repo : INSERT render_job (PENDING)
Worker -down-> Repo : SKIP LOCKED claim
Worker -down-> Core : mergeTemplate
Worker -down-> LO : convert
Worker -down-> Repo : UPDATE result/status
Worker -down-> Webhook : sendAsync(webhookUrl)

Cache -down-> Repo : findLatestActiveByName (cache miss)
Repo -down-> "PostgreSQL\n(Schema: production)" : JDBC

note right of Worker
  processNextJob: @Scheduled(every="2s")
  PostgreSQL SKIP LOCKED (ADR-008)
  cleanupJobs: @Scheduled(cron hourly)
  Stufe 1: result=null nach result-retention
  Stufe 2: DELETE nach record-retention
  Gilt für async- und sync-Einträge
end note

note right of LO
  LibreOffice ≥ 24
  headless: Format-Export
  ODT → PDF / RTF
end note
...
* Technical Functions nutzen Infrastructure Services (LibreOffice, Repository)
...
== Regression-Test ausfuehren ==

User -> WC : Sub-Tab "Coverage & Regression" öffnen
WC -> API : POST /templates/{id}/run-all-regressions
API -> API : renderPdf(template, testData) via HttpClient
API -> API : pdfComparisonService.areVisuallyIdentical()\n(mit und ohne ignoredPatterns)
API --> WC : List<RegressionResult>\n{passed, hasExpectedPdf, hasAcceptedDeviations}
WC --> User : Ergebnisse: ✅ Identisch / ✓ Akzeptierte Abweichungen\n❌ Abweichungen gefunden / — Kein Expected PDF

User -> WC : Auf abweichende Stelle klicken (Inline-Diff)
WC -> API : POST /regression-diff-pages
API --> WC : DiffPagesReport (Seiten-Bilder + markierte Blöcke)
WC --> User : Visueller Diff mit klickbaren Diff-Blöcken

User -> WC : Diff-Block klicken → "Nur diese Stelle ignorieren"
WC -> API : POST /ignore-block {pattern: "@area:page:x1:y1:x2:y2", scope: "this"}
API -> API : tds.ignoredPatterns.add(areaPattern)

note right of API
  Koordinaten-basierte Ignore-Patterns (@area:) ermöglichen
  positionsgenaues Ignorieren einzelner Stellen.
  Text-Patterns ignorieren alle Vorkommen über alle Tests.
end note

@enduml
----

=== Template-ID-basiertes Rendering mit Cache

*Szenario:* Produktive Anwendung generiert Dokument mit gespeichertem Template-Namen und automatischer Versions-Selektion (UC-10.1, TF-5)

[plantuml, laufzeit-template-name-rendering, svg]
----
@startuml
actor "Externe\nAnwendung" as App
participant "blocpress-render\nREST-API\n(öffentlich)" as Render
participant "RenderResource" as Resource
participant "TemplateCache" as Cache
participant "RenderEngine\n(TF-5)" as Engine
participant "LibreOfficeProcessor" as LO
participant "Repository\n(production DB)" as Repo

App -> Render : POST /api/render/{name}\n{data, outputType: pdf}
note right of App
    name = Template name (z.B. "Invoice", "PurchaseOrder")
    Automatische Auswahl der neuesten aktiven Version
end note

Render -> Resource : renderDocumentByName(name, request)
Resource -> Cache : getTemplateContentByName(name)

alt Cache Hit
    Cache --> Resource : byte[] (gecacht)
    note right of Cache
        Template im Cache vorhanden
        TTL: 10 min, Max: 100 Templates
    end note
else Cache Miss
    Cache -> Repo : findLatestActiveByName(name)
    note right of Repo
        TI-2: Direct DB read aus production-Datenbank
        Select highest version where validFrom <= now()
    end note
    alt Template found
        Repo --> Cache : Template (ODT binary)
        Cache -> Cache : store in cache (Caffeine)
        Cache --> Resource : byte[]
    else Template not found
        Repo --> Cache : TemplateNotFoundException
        Cache --> Resource : TemplateNotFoundException
        Resource --> App : 404 Not Found\n"Template not found"
    end alt
end alt

Resource -> Engine : mergeTemplate(templatePath, jsonData)
Engine -> Engine : Expand blocks, evaluate conditions,\nhandle loops, replace fields
Engine --> Resource : merged ODT
Resource -> LO : refreshAndTransform(merged, PDF)
LO -> LO : LibreOffice: ODT → PDF
LO --> Resource : PDF binary
Resource --> App : 200 OK (PDF binary)

note right of Cache
    UC-10.1: Template-Namen-basiertes Rendering mit Versioning
    - Auto-Versioning: mehrere Versionen pro Template-Name
    - validFrom-Feld: geplante Aktivierung/Rollout
    TI-2 DONE: Direct DB read aus production-Datenbank
    (kein HTTP-Fetch zu blocpress-workbench mehr)
end note

note right of Engine
    TF-5: Document generation
    Läuft in blocpress-render
    Liest aus production-Datenbank (TI-2 DONE)
end note
@enduml
----

=== Dokument über API generieren (asynchron, UC-23)

*Szenario:* Externe Anwendung generiert Dokument über den asynchronen Job-Queue-Endpunkt von blocpress-render (UC-23, TI-8). Implementiert mit PostgreSQL SKIP LOCKED statt Message Broker.

[plantuml, laufzeit-doc-generation, svg]
----
@startuml
participant "Externe\nAnwendung" as App
participant "blocpress-render\nAsyncRenderResource" as API
participant "RenderJobWorker\n(@Scheduled, alle 2s)" as Worker
participant "RenderEngine\n(TF-5)" as Engine
participant "LibreOfficeProcessor" as LO
participant "WebhookSender\n(VirtualThread)" as Webhook
participant "Repository\n(render_job Tabelle)" as Repo

App -> API : POST /api/render/jobs\n{templateName, data, outputType, webhookUrl?}
API -> Repo : INSERT render_job (status=PENDING)
Repo --> API : UUID jobId
API --> App : 202 Accepted\n{id, status: "PENDING"}

... Polling (optional) ...

App -> API : GET /api/render/jobs/{id}
API -> Repo : findById(jobId)
Repo --> API : {status: "PENDING"}
API --> App : {id, status: "PENDING"}

... Asynchrone Verarbeitung (Worker pollt alle 2s) ...

Worker -> Repo : UPDATE render_job SET status='PROCESSING'\nWHERE id = (SELECT id FROM render_job\nWHERE status='PENDING' ORDER BY created_at\nLIMIT 1 FOR UPDATE SKIP LOCKED)
note right of Repo
  SKIP LOCKED: Mehrere Worker-Instanzen
  können gleichzeitig Jobs claimem
  ohne sich gegenseitig zu blockieren
end note
Repo --> Worker : RenderJob (templateName, data, outputType)
Worker -> Engine : mergeTemplate(templatePath, jsonData)
Engine --> Worker : merged ODT
Worker -> LO : convert(mergedOdt, format)
LO --> Worker : result (PDF/ODT/RTF bytes)
Worker -> Repo : UPDATE render_job SET status='DONE',\nresult=<bytes>, updated_at=now()
Repo --> Worker : OK

alt webhookUrl vorhanden
  Worker -> Webhook : sendAsync(webhookUrl, {jobId, status})
...
App -> API : GET /api/render/jobs/{id}
API -> Repo : findById(jobId)
Repo --> API : {status: "DONE"}
API --> App : {id, status: "DONE"}

App -> API : GET /api/render/jobs/{id}/result
API -> Repo : findById(jobId)
Repo --> API : result bytes
API --> App : 200 OK\nContent-Type: application/pdf\nbinary stream

note right of API
  blocpress-render:
  Async Endpoints: /api/render/jobs/*
  Dashboard: GET /api/render/dashboard
  Liest aus Schema: production
end note
@enduml
----
...
=== Template Management Dashboard — Status-Filter und Workflow (UC-5)
...
Designer -> WC : APPROVED-Template → "Als Kopie" Button
...
UC -> UC : Prüfe: Name nicht vorhanden ✓
...
Designer -> WC : DRAFT-Template → "Aktualisieren" Button
WC -> WC : File-Dialog öffnen
Designer -> WC : Neue ODT-Datei auswählen
WC -> API : PUT /api/workbench/templates/{id}/content\n(multipart/form-data, file)
API -> UC : updateContent(templateId, newFile)
UC -> UC : Prüfe: Status = DRAFT ✓
UC -> UC : Kopiere newFile → content
UC -> Validator : validate(newContent)
...
note right of UC
  ...
  - Status-Filter (ALL, DRAFT, SUBMITTED, APPROVED, REJECTED, RETIRED)
  - Status-Übergänge mit Validierung
  - Approval-Dialog: validFrom (Datepicker) + reviewCycleYears → validUntil berechnet
  - Compliance-Badges: "Läuft ab" / "Abgelaufen" auf Template-Karten
  - Duplikation von APPROVED/RETIRED Templates
  - Re-Upload für DRAFT Templates
  - Kein User-Management (vereinfachte Version)
end note

@enduml
----

=== Stufenübergabe (workbench → production, Option A)

*Szenario:* Template durchläuft den Lebenszyklus von Entwicklung bis zur Produktionsfreigabe. TI-2 (Option A) implementiert: Direktes physisches Kopieren via REST-API. Kein blocpress-proof Modul in dieser Phase.

[plantuml, laufzeit-stufenuebergabe, svg]
----
@startuml
actor "Template-Designer" as Designer
participant "blocpress-workbench\n(Datenbank: workbench)" as WB
participant "blocpress-render\n(Datenbank: production)" as RD

== 1. Entwicklung und Test (workbench) ==

Designer -> WB : Template erstellen, bearbeiten\n(Status: DRAFT)
WB -> WB : TF-1: Validierung\nSpeichern in workbench.template
Designer -> WB : Status → SUBMITTED\n(PUT /api/workbench/templates/{id}/status)

== 2. Freigabe in workbench (Interim-Freigabeprozess) ==

Designer -> WB : Status → APPROVED\n(PUT /api/workbench/templates/{id}/status)
WB -> WB : template.validFrom = now()\npersist()

== 3. Automatisches Deploy: workbench → production (Kopie via REST) ==

WB -> RD : POST /api/render/templates/import\n{id, name, version, contentBase64, validFrom, validUntil}
RD -> RD : Upsert in production.template\n(physische Kopie der ODT-Binärdaten)
RD --> WB : 200 OK

== 4. Produktion ==

note over RD
  Template steht ueber die
  oeffentliche API zur
  Dokumentengenerierung bereit.
  findLatestActiveByName() filtert:
  validFrom <= now AND
  (validUntil IS NULL OR validUntil > now)
end note

== 5. Zurueckziehen (RETIRED) ==

Designer -> WB : Status → RETIRED\n(PUT /api/workbench/templates/{id}/status)
WB -> WB : template.validUntil = now()\nstatus = RETIRED\nES-Index loeschen
WB -> RD : DELETE /api/render/templates/import/{name}
RD -> RD : ProductionTemplate loeschen\nCache invalidieren
RD --> WB : 204 No Content

@enduml
----

=== Template-Ablehnung durch Reviewer

*Szenario:* Ein Reviewer lehnt ein zur Prüfung eingereichtes Template ab und hinterlässt eine Begründung für den Designer.

[plantuml, laufzeit-template-ablehnung, svg]
----
@startuml
actor "Reviewer" as R
actor "Template-Designer" as D
participant "blocpress-workbench\nREST-API" as API
participant "Repository" as Repo

== Template-Ablehnung ==

R -> API : POST /templates/{id}/reject\n{reason: "Bedingung 'vip == true' fehlt"}
API -> API : Validierung: status == SUBMITTED
API -> Repo : template.status = DRAFT\ntemplate.rejectionReason = reason\ntemplate.rejectedAt = now()
Repo --> API : OK
API --> R : 200 {id, status: DRAFT}

== Designer sieht Ablehnung ==

D -> D : Workspace öffnen
note right of D
  Ablehnungs-Banner sichtbar:
  "⚠ Abgelehnt am 07.03.2026:
   Bedingung 'vip == true' fehlt"
end note

D -> D : Template korrigieren
D -> API : PUT /templates/{id}/status {SUBMITTED}
API -> Repo : status = SUBMITTED\nrejectionReason = null\nrejectedAt = null
note right of API
  Ablehnungsgrund wird beim Re-Submit
...
*Szenario:* Täglicher Scheduler in blocpress-workbench prüft ablaufende Templates (TF-7, UC-12). Implementiert: `ComplianceReviewScheduler` täglich 08:00, konfigurierbare Vorlaufzeit (`blocpress.compliance.review-lead-days`, Default 60 Tage).

[plantuml, laufzeit-compliance-check, svg]
----
@startuml
participant "Scheduler\n(Quarkus @Scheduled)" as Scheduler
participant "blocpress-workbench\nComplianceReviewScheduler\n(TF-7)" as CM
participant "Repository\n(Schema: workbench)" as Repo
participant "Reviewer" as R

Scheduler -> CM : checkDueForReview()\n(täglich 08:00 Uhr)
CM -> Repo : Template.list(\n  status=APPROVED AND\n  validUntil IS NOT NULL AND\n  validUntil <= now + 60d)
Repo --> CM : List<Template>

alt Templates gefunden
  CM -> CM : LOG.warn("{} Template(s) laufen\ninnerhalb von 60 Tagen ab: …")
end

note right of CM
  TF-7: Compliance-Reviews überwachen
  Läuft in blocpress-workbench
  Schema: workbench
  Konfigurierbar: BLOCPRESS_COMPLIANCE_LEAD_DAYS
end note

== Review-Ansicht (UC-12) ==

R -> Repo : GET /api/workbench/templates/due-for-review
Repo --> R : List<TemplateDetails> (mit validUntil, reviewCycleYears)
R -> R : "Neu freigeben" → Approval-Dialog\n(neues validFrom + reviewCycleYears)
@enduml
----
...
=== Quickstart (Single Container — blocpress-studio)

Für lokale Entwicklung und schnellen Einstieg steht das All-in-one Image `flaechsig/blocpress-studio:latest` zur Verfügung.
Es bündelt alle drei Laufzeitmodule und PostgreSQL in einem einzigen Docker-Container, verwaltet durch *supervisord*.

*Start:*

[source,bash]
----
docker run -d -p 8080:8080 -p 8081:8081 --name blocpress-studio flaechsig/blocpress-studio:latest
----

[plantuml, deployment-quickstart, svg]
----
@startuml
skinparam componentStyle rectangle

node "Docker Container: flaechsig/blocpress-studio" {
  component "blocpress-studio\n(Quarkus, Port 8080)\nPortal-Shell + API-Proxy\n/proxy/* + /api/*" as Studio
  component "blocpress-workbench\n(Quarkus, intern Port 8082)\nTemplate-Entwicklung" as WB
  component "blocpress-render\n(Quarkus, Port 8081)\nDokumentengenerierung" as RD
  database "PostgreSQL 16\n(intern, nicht exponiert)\nDBs: workbench, production" as DB
  component "supervisord" as SUP
}

Studio -down-> WB : REST (intern)\n/api/* Proxy
WB -down-> DB : JDBC (workbench)
RD -down-> DB : JDBC (production)
WB ..> RD : REST (Vorschau / Deploy)
SUP -[dashed]-> Studio
SUP -[dashed]-> WB
SUP -[dashed]-> RD

note right of Studio
  Token-Guard: Workbench-UI
  nur mit gesetztem JWT
  zugänglich
end note
...
| 8080
| Studio (Workbench UI + API-Proxy → Workbench intern)

| 8081
| Render API (Swagger UI, öffentlich)
|===

*Konfiguration via ENV (alle optional — Defaults für Quickstart eingebaut):*
...
| JWT Public Key (RSA). Default: eingebundener Dev-Key.

| `MP_JWT_VERIFY_ISSUER`
| JWT Issuer. Default: `https://blocpress.dev`.

| `WORKBENCH_URL`
| Interne Workbench-URL für Proxy (Default: `http://localhost:8082`).

| `RENDER_URL`
| Interne Render-URL für Vorschau-Generierung (Default: `http://localhost:8081`).
|===

NOTE: blocpress-studio proxiert im Quickstart-Modus alle Browser-Requests auf `/api/*` transparent an blocpress-workbench (intern Port 8082). Port 8082 ist nach außen nicht exponiert. Static-File-Caching ist deaktiviert, damit Container-Rebuilds sofort im Browser sichtbar sind.

NOTE: Das Quickstart-Image verwendet das Dev-JWT aus `docs/samples/quickstart/token.txt`. Für Produktionseinsatz eigenes Schlüsselpaar konfigurieren und `MP_JWT_VERIFY_PUBLICKEY` / `MP_JWT_VERIFY_ISSUER` überschreiben.
...
=== Build- und Release-Pipeline (GitHub Actions)

Zwei Workflows steuern CI und Release vollautomatisch.

==== CI-Workflow (`ci.yml`)

Wird bei jedem Push auf `main` oder `workbench-*` sowie bei Pull Requests ausgelöst.

* Unit-Tests: `mvn clean test -pl blocpress-core,blocpress-workbench`
* Integration-Tests mit Testcontainers (Docker): `mvn verify -DskipUTs`

==== Release-Workflow (`release.yml`)

Wird manuell per `workflow_dispatch` ausgelöst — entweder in der GitHub UI oder über die Kommandozeile:

[source,bash]
----
mvn validate -Ptrigger-release -Drelease.version=2.3.0 -Drelease.next=2.4.0
----

Der Workflow führt folgende Schritte sequenziell aus:
...
| Version setzen
| `mvn versions:set -DnewVersion=X.Y.Z` in allen pom.xml + Landing Page (`docs/index.html`) aktualisieren, committen

| Build & Test
| Unit-Tests + Integration-Tests (inkl. Testcontainers)

| Maven Central
| `blocpress-core` signiert (GPG) und deployed via `central-publishing-maven-plugin` (`-Prelease`)

| Docker Hub
| Quickstart-Image gebaut und gepusht als `:X.Y.Z` + `:latest`; Docker Hub README aus `docs/dockerhub.md` aktualisiert

| Git-Tag
| `vX.Y.Z` gesetzt und gepusht

| Merge
| Workbench-Branch → `main` (no-ff), Branch danach gelöscht

| Nächste Version
| `mvn versions:set -DnewVersion=X.Y+1.0-SNAPSHOT` auf `main` committen

| GitHub Release
| Erstellt mit dem Abschnitt `[X.Y.Z]` aus `CHANGELOG.md` als Release-Notes
|===
...
| Lokaler GPG-Key (`gpg --armor --export-secret-keys`)
...
* `main` trägt immer eine SNAPSHOT-Version (z.B. `2.3.0-SNAPSHOT`) — sichtbarer Indikator für Entwicklungsstand
* Release-Tags (`v2.2.0`) zeigen auf den Commit ohne `-SNAPSHOT`
* Nur `blocpress-core` wird auf Maven Central veröffentlicht — die Quarkus-Apps (render, workbench, studio) sind Deployable-Artefakte, keine Libraries
...
package "Schema: workbench" #LightBlue {
  ENTITY "Template\n(Entwurf)" as TWB {
    ...
    + name: String
    + version: Integer
    + status: String (DRAFT/SUBMITTED/APPROVED/REJECTED/RETIRED)
    + content: bytea
...
package "Schema: proof [geplant - blocpress-proof]" #LightGreen {
  ENTITY "Template\n(Prüfung)" as TPR {
    + id: Long
    + name: String
    + version: Integer
...
package "Schema: admin [geplant - blocpress-admin]" #LightCoral {
  ENTITY Benutzer {
    + id: Long
    + name: String
    + email: String
...
NOTE: Die Template-Tabelle existiert in den Datenbanken `workbench` (vollständig mit Status, Validierung, Compliance-Feldern) und `production` (vereinfacht: nur Pflichtfelder + `valid_until`). TI-2 (Option A) ist implementiert: Direktes physisches Kopieren via REST-API (`POST /api/render/templates/import`, `DELETE /api/render/templates/import/{name}`). UC-12 (Compliance Review) ist implementiert: `valid_until` und `review_cycle_years` auf beiden Tabellen; abgelaufene Templates werden in `findLatestActiveByName()` ausgeblendet (404). Die Schemata `proof` und `admin` sind für zukünftige Module geplant.
...
* Im Quickstart: JWT wird in LocalStorage persistiert (`bp-token-input` Komponente) und beim Neuladen wiederhergestellt
...
* *Reviewer-Freigabe:* Organisatorisch durchgesetzt. In der Interim-Implementierung (TI-2 Phase 1) findet die Freigabe in blocpress-workbench statt; technische Rollenprüfung ist geplant für blocpress-proof.
* *HTTPS/TLS:* Alle externen Verbindungen verschlüsselt
* *JWT-Weitergabe:* Im Quickstart-Modus via LocalStorage (`bp-jwt`-Key, `bp-token-input` Komponente) → Property an Web Component. Für Produktionsbetrieb: JWT-Handling über externen IdP, kein LocalStorage.
...
| `production`
| blocpress-render
| Template (Freigegeben, E-1), RenderJob (async + sync Audit-Log, E-3) — zweistufige Bereinigung via stündlichem Cleanup-Job
...
*Dynamic Import (Implementierung):*

[source,javascript]
----
// bp-workbench.js wird nicht direkt von blocpress-workbench geladen,
// sondern über WorkbenchProxyResource (/proxy/bp-workbench.js).
//
// WHY: Dynamic import() unterliegt striktem CORS-Enforcement im Browser.
// Der Proxy leitet den Request server-seitig an blocpress-workbench weiter
// → beide Seiten teilen denselben Origin → kein CORS nötig.
// Vorteil: kein CORS-Header-Management, kein Preflight.
await import(`/proxy/bp-workbench.js?v=${Date.now()}`);
//
// WorkbenchProxyResource (/proxy/bp-workbench.js):
//   - Fetcht bp-workbench.js von workbench.url (konfigurierbar, default: localhost:8082)
//   - Validiert Antwort (kein HTML — hot-reload-safe)
//   - Gibt Cache-Control: no-store zurück
//   - Retry-Logik (5 Versuche, 1.5s Abstand)
----
...
=== Template-Verarbeitung mit odfdom

Die Rendering-Pipeline arbeitet rein in-memory mit dem Apache ODF Toolkit (odfdom) —
LibreOffice wird ausschließlich für den abschließenden Format-Export (ODT→PDF/RTF) als
Headless-Prozess gerufen. Es gibt keine LibreOffice-UNO-API-Abhängigkeit im Kernpfad.

*Rendering-Pipeline (`RenderEngine.mergeTemplate`):*

[source,java]
----
// Einstiegspunkt: blocpress-core
public static TemplateDocument mergeTemplate(URL template, JsonNode data) {
    TemplateDocument doc = TemplateDocument.load(template);

    // Schritt 1: Text-Block-Expansion
    // Externe ODT-Dateien (text:section-source) werden inline expandiert
    expandTextBlocks(doc);

    // Schritt 2: Bedingungsauswertung
    // text:section / text:conditional-text / text:p / text:span
    // mit JEXL-Ausdrücken gegen JSON evaluiert
    evaluateConditions(doc, data);

    // Schritt 3: Loop-Verarbeitung
    // Sections und Tabellenzeilen mit Array-Feldern werden
    // für jedes Array-Element dupliziert; Felder erhalten
    // indizierten Namen (z.B. positionen.0.bezeichnung)
    processLoops(doc, data);

    // Schritt 4: Feld-Ersetzung
    // text:user-field-get / text:variable-get werden durch
    // Werte aus dem JSON (Dot-Notation-Pfade) ersetzt
    replaceFields(doc, data);

    return doc;
}
----

*Format-Export (nur blocpress-render):*

[source,java]
----
// ODT-Dokument als Bytes speichern
byte[] odtBytes = renderEngine.mergeToBytes(template, data);

// Konvertierung ODT→PDF oder ODT→RTF über LibreOffice Headless
// LibreOfficeProcessor startet einen soffice-Prozess via ProcessBuilder
byte[] result = libreOfficeProcessor.convert(odtBytes, outputFormat);
----

*Schlüsselklassen:*

* `RenderEngine` — Orchestriert die vier Pipeline-Schritte
* `OdtTemplateDocument` — Wraps `OdfTextDocument` (odfdom), implementiert `TemplateDocument`
* `OdtTemplateElement` — Wraps ODF-Elemente, wertet Bedingungen via `JexlConditionEvaluator` aus
* `JexlConditionEvaluator` — Wertet JEXL-Ausdrücke gegen JSON-Daten aus
* `LibreOfficeProcessor` — Spawnt `soffice --headless` für PDF/RTF-Export (nur in blocpress-render)
...
* LibreOffice-Version fixiert (≥ 24) für stabile PDF-Ausgabe
...
* Negativ: PostgreSQL-Backup größer, bei > 5.000 Dokumenten wäre S3-Migration sinnvoll
...
=== ADR-003: Synchrone Dokumentengenerierung (Standard-Endpunkte)

*Status:* Akzeptiert (ergänzt durch ADR-008 für asynchronen Modus)

*Kontext:*

Dokumentengenerierung kann mehrere Sekunden dauern. Soll der REST-API-Call blockieren oder asynchron sein?

*Entscheidung:*

Synchrone Verarbeitung als Standard: API-Call wartet bis das Dokument fertig generiert ist und gibt das Ergebnis direkt zurück. Für Batch- und Langläufer-Szenarien steht zusätzlich ein asynchroner Job-Queue-Modus zur Verfügung (ADR-008).

*Begründung:*

* *Einfachheit:* Für Einzel-Generierungen kein asynchroner Mechanismus nötig
* *Zuverlässigkeit:* Direkte Fehlerbehandlung möglich
* *Ausreichend:* Für Workbench-Vorschau und produktive Einzelgenerierung ist synchrones Rendering schnell genug

*Konsequenzen:*

* Positiv: Einfachere Architektur für den Standardfall, direktes Feedback
* Negativ: Bei sehr langen Generierungen (> 60s) können HTTP-Timeouts auftreten
* Mitigation: Timeout auf 60 Sekunden konfiguriert; Batch-Szenarien nutzen den asynchronen Job-Modus (ADR-008)

=== ADR-004: Elasticsearch für Content Search

*Status:* Umgesetzt (v2.2)
...
Elasticsearch 8.11 via `quarkus-elasticsearch-rest-client` (Low-Level REST Client, kein Hibernate Search ORM).
...
*Begründung für Low-Level REST Client (statt Hibernate Search ORM):*

* Template-Entity ist JPA-seitig stabil — keine ORM-Annotierungen nötig
* ODT-Textextraktion braucht Custom-Code (`OdtTextExtractor` in blocpress-core) — direkter Client gibt volle Kontrolle über Index-Schema
* Einfacher mit Testcontainers testbar

*Konsequenzen:*

* Positiv: Exzellente Suchperformance und -qualität, volle Kontrolle über Query-Format
* Positiv: `OdtTextExtractor` in blocpress-core wiederverwendbar für andere Module
* Negativ: Zusätzliche Infrastruktur-Komponente, höhere Komplexität im Quickstart-Image
* Risiko: Eventual Consistency (Index kann wenige Sekunden nach Upload durchsuchbar sein)
...
=== ADR-008: PostgreSQL SKIP LOCKED statt Message Broker für asynchrones Rendering

*Status:* Umgesetzt (v2.2)

*Kontext:*

UC-23 erfordert asynchrone Dokumentengenerierung für Batch-Läufe und Großdokumente. Optionen: Apache Kafka, RabbitMQ, oder PostgreSQL-basierte Job-Queue.

*Entscheidung:*

PostgreSQL-basierte Job-Queue mit `SELECT ... FOR UPDATE SKIP LOCKED LIMIT 1` (Tabelle `render_job` im production-Schema). `@Scheduled`-Worker in Quarkus pollt alle 2s und claimed den nächsten freien Job atomar. Webhook-Callbacks via Java Virtual Threads (Fire-and-Forget).

*Begründung:*

* *Keine zusätzliche Infrastruktur:* blocpress-render hat bereits eine PostgreSQL-Verbindung — kein separater Kafka/RabbitMQ-Container nötig
* *SKIP LOCKED:* Konkurrenzsicheres Job-Claiming: Mehrere Worker-Instanzen können gleichzeitig Jobs claimem ohne Locks oder Deadlocks
* *Einfachheit:* `@Scheduled` + Native Query — kein Reactive Messaging Framework, kein Schema-Registry, keine Consumer-Group-Konfiguration
* *Ausreichend für aktuelle Last:* Bei < 100 Jobs/min ist eine 2s-Poll-Latenz akzeptabel
* *Persistenz:* Job-Ergebnisse (result bytea) und Fehler (error_message) bleiben dauerhaft in der DB — kein separates Result-Storage nötig

*Konsequenzen:*

* Positiv: Kein neuer Infrastruktur-Overhead, transaktionale Job-Übergabe, einfache Fehlerdiagnose via SQL
* Negativ: Polling-Overhead (2s Intervall), bei > 1.000 Jobs/min wäre Kafka performanter
* Mitigation: Poll-Intervall via ENV `BLOCPRESS_ASYNC_POLL_INTERVAL` konfigurierbar; Migration zu Kafka möglich ohne API-Änderung

*Job-Lifecycle und Bereinigung:*

Jobs durchlaufen zwei Löschstufen (stündlicher Cleanup-Job `@Scheduled(cron = "0 0 * * * ?")`):

. *result-retention* (Default 24h, ENV `BLOCPRESS_ASYNC_RESULT_RETENTION`): `result`-Bytes werden auf `null` gesetzt — Speicher wird freigegeben, Metadaten bleiben für Audit
. *record-retention* (Default 7 Tage, ENV `BLOCPRESS_ASYNC_RECORD_RETENTION`): Gesamter Datensatz wird gelöscht

Synchrone Render-Aufrufe (`POST /api/render/{name}`) schreiben ebenfalls einen `RenderJob`-Datensatz als Audit-Log (`result = null`, da Dokument direkt per HTTP zurückgegeben wurde). Die Bereinigung gilt für sync- und async-Einträge gleichermaßen.

=== ADR-007: WebDAV-Endpoint für Baustein- und Template-Zugriff aus LibreOffice

*Status:* Akzeptiert

*Kontext:*

Template-Designer arbeiten in LibreOffice und binden Bausteine als verknüpfte Abschnitte (`text:section-source`) in Templates ein. Diese Verknüpfungen erfordern eine HTTP-URL zum Baustein-ODT. Der Zugriff muss sowohl zur Designzeit (LibreOffice lädt den Baustein) als auch zur Renderzeit (`RenderEngine.expandTextBlocks()` fetcht `text:section-source`-URLs) funktionieren. Eine rein browserbasierte Upload-/Download-Lösung würde manuelles URL-Management erfordern und unterbricht den natürlichen Arbeitsfluss in LibreOffice.

*Entscheidung:*

blocpress-workbench exponiert einen WebDAV-Endpoint (`/api/webdav/`) als JAX-RS-Resource:

* DRAFT-Versionen unter `/api/webdav/{bausteine|templates}/{name}.odt` — read-write (GET + PUT)
* APPROVED-Versionen unter `/api/webdav/released/{bausteine|templates}/{name}.odt` — read-only (GET)
* PROPFIND für Verzeichnislistings (207 Multi-Status XML) implementiert via eigener `@PROPFIND`-Annotation (JAX-RS §3.3 Custom HTTP Methods)
* DAV Level 1 — keine externe WebDAV-Bibliothek, reine JAX-RS-Implementierung
* PUT erzeugt bei unbekanntem Namen automatisch einen neuen DRAFT-Eintrag (WebDAV create-or-update)

LibreOffice verbindet sich per `dav://workbench:8081/api/webdav/bausteine/`. Filesystem-Mounts über GNOME Files oder `davfs2` ermöglichen transparentes Bearbeiten. Zur Renderzeit verwendet blocpress-core die `/released/`-URLs direkt über `java.net.URL.openStream()`.

*Begründung:*

* *Einfachheit für Designer:* Standard-Dateisystem-Integration ohne Browser-Uploads
* *Renderzeit-Kompatibilität:* `RenderEngine` nutzt bereits `java.net.URL` für `text:section-source` — WebDAV-URLs werden nativ unterstützt
* *Statusbewusstsein:* `/released/` trennt DRAFT von APPROVED — LibreOffice lädt gezielt die freigegebene Version
* *Keine Infrastruktur-Zusatzabhängigkeit:* Kein separater WebDAV-Server nötig

*Konsequenzen:*

* Positiv: Nahtlose LibreOffice-Integration, Designzeit- und Renderzeit-Zugriff über gleiche URL-Basis, Filesystem-Mount-Kompatibilität (GNOME Files, davfs2)
* Negativ: Nur DAV Level 1 (kein LOCK/UNLOCK) — parallele Bearbeitung ohne Sperrmechanismus möglich
* Mitigation: DRAFT-Status als softwareseitiger Schutz; bei konkurrierendem Zugriff gewinnt der letzte PUT (letzte Schreibt-gewinnt-Semantik akzeptabel für ein Einzelnutzer-Entwicklungswerkzeug)
...
| Organisatorisch sichergestellt durch Rollentrennung. Technisch: kein automatischer Schutz in der Interim-Implementierung (TI-2 Phase 1). Geplant: blocpress-proof mit Rollenprüfung.

| *Skalierbarkeit*
| Horizontale Skalierung
| 150 gleichzeitige Dokumentengenerierungen
| Kubernetes skaliert auf 5 blocpress-render Pods, Load Balancer verteilt Last
...
=== Test-Strategie und Metriken

**Grundprinzipien:**

Die Test-Strategie von blocpress folgt einem pragmatischen Ansatz, der **funktionale Korrektheit vor Instrumentation-Coverage** priorisiert:

1. **Integration Tests mit echten Datenbanken:** H2 in-memory Datenbanken für Test-Isolation ohne Mocks
2. **Echtheit über Metrik-Spielerei:** Tests durchlaufen die komplette Schicht (HTTP → REST → Business Logic → Database)
3. **Fehler-Discovery durch echte Szenarien:** Datenbank-Tests decken Fehler auf, die Mocks nie finden würden
4. **Fokus auf Regressionstests:** Validierung, dass identische Inputs identische Outputs produzieren

**Test-Klassifizierung:**

[cols="1,2,3,2" options="header"]
|===
| Typ | Zweck | Technologie | Anteil

| *Unit Tests*
| Logik einzelner Methoden isoliert testen (Validator, Parser, etc.)
| JUnit 5, Mockito
| 60%

| *Integration Tests*
| Komplette Workflows mit echten Abhängigkeiten (DB, REST-Calls)
| JUnit 5, H2, REST-Assured, Quarkus @QuarkusTest
| 40%

| *Regression Tests*
| Identische Inputs → identische Outputs (Determinismus-Validierung)
| PDF-Byte-Vergleiche, Expected Result PDFs
| Subset von Integration Tests
|===

**Metriken und Erkenntnisse (blocpress-workbench):**

- **Test-Umfang:** 173 Tests (105 Unit + 68 Integration)
- **Automatisierte Bugs gefunden:** 4 echte Fehler in Error Handling (alle durch H2 Integration Tests)
  * `createTestDataSet()` für nicht-existentes Template: War 500, korrigiert zu 404
  * `updateTestDataSet()` für nicht-existentes Template: War 500, korrigiert zu 404
  * `deleteTestDataSet()` für nicht-existentes Template: War 204, korrigiert zu 404
  * `saveExpectedPdf()` für nicht-existentes Template: War 500, korrigiert zu 404
- **JaCoCo Instruction Coverage:** 28% (650 / 2,251 instructions)
- **JaCoCo Method Coverage:** 92% (23/25 Methoden)
- **JaCoCo Cyclomatic Complexity:** 96% (68/71)

**Wichtiger Hinweis zur Coverage-Metrik:**

REST-API-Code (TemplateResource) wird von JaCoCo zu nur 1% gemessen, obwohl 100% der Pfade via Integration Tests abgedeckt sind. Grund: Quarkus startet einen HTTP-Server, Code läuft in separatem ClassLoader, HTTP-Marshalling wird nicht instrumentiert.

**Konsequenz:** JaCoCo-Metriken für REST-APIs sind **nicht aussagekräftig**. Der pragmatische Ansatz ist:
- Integration Tests **funktionale Korrektheit** verifizieren
- Unit Tests Kernlogik isoliert validieren
- Coverage-Tools für reine Business-Logic-Module nutzen

**Best Practice Erkenntnisse:**

1. ✅ H2-basierte Integration Tests finden echte Fehler (Beweis: 4 Bugs entdeckt und korrigiert)
2. ✅ Direkte Methodenaufrufe in @QuarkusTest sind transaktional möglich
3. ✅ Mock-basierte Tests übersehen Fehler, die in echten Abhängigkeiten auftauchen
4. ✅ Determinismus-Tests mit Expected Result PDFs validieren Qualitätsziel "Zuverlässigkeit"
5. ❌ JaCoCo-Coverage für REST-APIs ist Cargo Cult Metrics - nicht hilfreich

**Zukunftsbild:**

Nach Implementierung von TI-2 (Multi-Schema), werden Integration Tests mit direktem DB-Zugriff aufgebaut. REST-HTTP-Layer wird dann vollständig getestet, neue Integration Tests mit echtem PostgreSQL werden Produktions-Szenarien validieren.
...
- Migration zu S3 als zukünftige Option
...
| *blocpress-workbench*
| SCS-Modul für Template-Entwicklung: Upload, Validierung, Bearbeitung, Bausteinverwaltung, Content Search, WebDAV-Endpoint. Arbeitsumgebung des Template-Designers. Schema: `workbench`.

| *WebDAV*
| Web Distributed Authoring and Versioning (RFC 4918) — HTTP-Erweiterung für kollaborativen Dateizugriff. blocpress-workbench implementiert DAV Level 1 unter `/api/webdav/` für direkten Baustein- und Template-Zugriff aus LibreOffice. DRAFT-Versionen read-write, APPROVED-Versionen (unter `/released/`) read-only.
...
| *Stufenübergabe*
| Kopie-basierte Überführung von Templates zwischen Datenstufen. Implementiert (TI-2 Option A): workbench → production via `POST /api/render/templates/import` (automatisch bei APPROVED-Status). Geplant: workbench → proof (Reviewer-Prüfung) → production.

| *Template*
| LibreOffice-Dokument (ODT) mit Platzhaltern (User-Fields), Wiederholungsgruppen und Bedingungen. Existiert in zwei Datenbanken (workbench/production, TI-2 implementiert). Ziel: drei Stufen (workbench/proof/production). Entity E-1.
...
| *Reviewer-Freigabe*
| Sicherheitskonzept: Templates müssen von Reviewer-Rolle freigegeben werden. Organisatorisch durchgesetzt (nicht technisch erzwungen). Bei Personen mit Developer + Reviewer Rollen: organisatorisches Qualitätssicherungs-Problem.
