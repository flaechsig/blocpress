...
*Szenario:* Produktive Anwendung generiert Dokument mit gespeicherter Template-ID statt Binary-Upload (UC-10, TF-5)

[plantuml, laufzeit-template-id-rendering, svg]
----
@startuml
actor "Externe\nAnwendung" as App
...
App -> Render : POST /api/template/{id}/render\n{data, outputType: pdf}
Render -> Resource : renderDocumentById(uuid, request)
Resource -> Cache : getTemplateContent(templateId)
...
alt Cache Hit
    ...
    Cache -> Workbench : GET /api/workbench/templates/{id}/content
    Workbench -> Repo : findById(templateId)
    Repo --> Workbench : Template entity
