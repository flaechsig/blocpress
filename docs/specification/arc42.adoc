...
alt Cache Hit
    ...
    Cache -> Workbench : GET /api/workbench/templates/by-name/{name}/content
    Workbench -> Repo : findLatestActiveByName(name)
    note right of Repo
        UC-10.1: Select highest version\nwhere status=APPROVED AND validFrom <= now()
    end note
    Repo --> Workbench : Template entity (neueste aktive Version)

    alt Status = APPROVED && validFrom <= now()
        Workbench --> Cache : 200 OK (ODT binary)
        Cache -> Cache : store in cache (Caffeine)
        Cache --> Resource : byte[]
    else Status ≠ APPROVED || validFrom > now()
        Workbench --> Cache : 403 Forbidden
        Cache --> Resource : TemplateNotFoundException
        Resource --> App : 403 Forbidden\n"Template not approved or not yet active"
    end alt

    alt Template not found
        Workbench --> Cache : 404 Not Found
        Cache --> Resource : TemplateNotFoundException
...
note right of Cache
    ...
    Interim-Lösung: HTTP-Fetch zu blocpress-workbench
    Zukünftig (TI-2): Direct DB read aus production-Schema
end note

note right of Engine
    TF-5: Document generation
    Läuft in blocpress-render
    Schema: production (zukünftig)
end note
@enduml
----
