...
*Description:*
blocpress-proof soll einen mehrstufigen Freigabeprozess mit Reviewer-Freigabe implementieren (organisatorisch durchgesetzt: Template-Designer ≠ Prüfer). Templates dürfen nur nach erfolgreicher Prüfung in das production-Schema übernommen werden. +
...
*Description:*
Web Component `<bp-workbench>` zum Hochladen, Bearbeiten, Anzeigen und Verwalten von Templates. Wird von Template-Designern genutzt. Geladen via Dynamic Import in blocpress-studio. Bietet Dashboard-Ansicht mit Status-Filtern und Workflow-Funktionen (UC-5).
...
*Description:*
Bestandteil des Web Components `<bp-proof>` zur Prüfung und Freigabe von Templates. Wird von Prüfern genutzt. Geladen via Dynamic Import in blocpress-studio.
...
*Description:*
Web Component `<bp-admin>` zur Verwaltung von Benutzern, Rollen und System-Konfiguration. Wird von Administratoren genutzt. Geladen via Dynamic Import in blocpress-studio.
...
- `<bp-workbench>` Web Component (geladen in blocpress-studio)
- blocpress-workbench (SCS-Modul)
- _blocpress-proof (geplant, noch nicht implementiert)_

*Main Scenario:*

1. Template-Designer wählt Template auf UI-1 aus
2. Template-Designer wählt Aktion "Zur Freigabe einreichen"
3. `<bp-workbench>` sendet Anfrage über REST-API (PUT /api/workbench/templates/{id}/status?status=APPROVED) an blocpress-workbench
4. blocpress-workbench ändert Template-Status auf APPROVED (Stufenübergabe innerhalb workbench)
5. _[Zielzustand mit blocpress-proof] blocpress-proof wird Template empfangen und Freigabeprozess starten — noch nicht implementiert_
6. blocpress-workbench bestätigt erfolgreiche Einreichung an `<bp-workbench>`
7. `<bp-workbench>` zeigt Bestätigung auf UI-1 an
8. Template-Status ist für Designer auf UI-1 als APPROVED sichtbar

*Alternative Scenarios:*

_[Geplant für blocpress-proof]_
...
- `POST /api/render/template/upload` - Template mit JSON-Daten zusammenführen und als ODT/PDF/RTF zurückgeben (Multipart)
- Parameter: template (binary), data (JSON)
- `POST /api/render/template` - Template mit JSON-Daten zusammenführen (Base64-kodiert) und als ODT/PDF/RTF zurückgeben
- Parameter: requestBody (template: base64, data: JSON, outputType: pdf|rtf|odt)
- `POST /api/render/{name}` - Dokument mit gespeichertem Template (Name) und JSON-Daten generieren (UC-10.1)
- Parameter: name (String path), requestBody (data: JSON, outputType: pdf|rtf|odt)
- Nur APPROVED Templates zugänglich
- Template-Content wird gecacht (10 min TTL)
- Automatische Selektion der neuesten aktiven Version basierend auf validFrom-Datum
- Response: Binary Dokument im gewünschten Format

*Interne API (blocpress-workbench, Port 8081):*

Template-Management:

- `POST /api/workbench/templates` - Template hochladen (auto-versioning nach Name, UC-10.1)
- `GET /api/workbench/templates/{id}` - Template-Binärdaten abrufen (Download)
- `GET /api/workbench/templates` - Template-Liste mit Status und Validierung abrufen (nur neueste Version pro Name)
- `GET /api/workbench/templates/{id}/details` - Template-Details mit Validierungsergebnis
- `GET /api/workbench/templates/{id}/content` - Template-Content für blocpress-render abrufen (nur APPROVED, UC-10)
- `GET /api/workbench/templates/by-name/{name}/content` - Template-Content nach Name abrufen (nur neueste APPROVED Version, UC-10.1)
- `POST /api/workbench/templates/{id}/submit` - Template zur Freigabe einreichen (löst Stufenübergabe an proof aus)
- `PUT /api/workbench/templates/{id}/status` - Template-Status ändern (DRAFT→SUBMITTED, etc., UC-5); setzt validFrom=now() bei APPROVED Transition (UC-10.1)
- `POST /api/workbench/templates/{id}/duplicate` - Produktives Template als DRAFT-Kopie erstellen (UC-5)
- `PUT /api/workbench/templates/{id}/content` - Template-Inhalt aktualisieren/neu hochladen (UC-5)
- `DELETE /api/workbench/templates/{id}` - Template löschen
- `GET /api/workbench/search` - Nach Begriff suchen

TestDataSet-Management (UC-20, UC-21, TF-8, UC-11):

- `GET /api/workbench/templates/{id}/testdata` - Alle TestDataSets für Template abrufen
- `POST /api/workbench/templates/{id}/testdata` - Neues TestDataSet erstellen {name, testData}
- `PUT /api/workbench/templates/{id}/testdata/{testDataId}` - TestDataSet aktualisieren
- `DELETE /api/workbench/templates/{id}/testdata/{testDataId}` - TestDataSet löschen
- `POST /api/workbench/templates/{id}/testdata/{testDataId}/save-expected` - PDF als Expected Result speichern
- `GET /api/workbench/templates/{id}/testdata/{testDataId}/expected-pdf` - Expected PDF abrufen
...
- `POST /api/render/templates/import` - Template von workbench in production-DB importieren (TI-2 implementiert)
- _[geplant] `POST /api/proof/stage/receive` - Template von workbench empfangen_
...
====== UC-10.1: Template-Versionierung mit validFrom-Datum

*Implementierungsdetails:*

Template-Versionierung ermöglicht es, mehrere Versionen desselben Templates (gleicher Name) parallel zu verwenden und geplante Rollouts über ein `validFrom`-Feld zu unterstützen.

*Funktionsweise:*

1. **Auto-Versioning beim Upload:** Jedes Mal, wenn ein Template mit einem bereits existierenden Namen hochgeladen wird, erhält es automatisch die nächste Versionsnummer. Beispiel: Erstes Upload → v1, zweites Upload → v2, drittes Upload → v3.

2. **Unique Constraint:** Die Kombination (name, version) ist eindeutig in der Datenbank. Dies verhindert doppelte Versionen desselben Namens.

3. **validFrom-Feld:** Wird automatisch auf `now()` gesetzt, wenn ein Template den Status APPROVED erhält. Dies ermöglicht geplante Aktivierungen: Administratoren können das Feld auf ein zukünftiges Datum setzen, um die Aktivierung zu verschieben.

4. **Neueste aktive Version auswählen:** Beim Rendern wird automatisch die höchste Versionsnummer gewählt, deren `validFrom` ≤ now() ist. Beispiel: Wenn v1, v2, v3 alle APPROVED sind, aber v3 mit einem zukünftigen validFrom-Datum versehen wurde, wird v2 verwendet.

5. **Keine Löschung:** Alte Template-Versionen bleiben in der Datenbank erhalten. Dies ermöglicht ein Rollback auf frühere Versionen, falls nötig.

*API-Auswirkungen:*

- `POST /api/workbench/templates` — Auto-Versioning basierend auf Name
- `GET /api/workbench/templates` — Zeigt nur die neueste Version pro Template-Name an
- `GET /api/workbench/templates/by-name/{name}/content` — Ruft die neueste aktive APPROVED Version ab
- `POST /api/render/{name}` — Rendert mit der neuesten aktiven Version
- `PUT /api/workbench/templates/{id}/status` — Setzt `validFrom` bei Transition zu APPROVED

*Cache-Strategie:*

blocpress-render nutzt einen Cache (Caffeine, 10 min TTL, 100 Template max) für Template-Content, um HTTP-Requests an blocpress-workbench zu minimieren. Der Cache wird invalidiert, wenn Administratoren ein zukünftiges validFrom-Datum setzen (manueller Invalidierungsmechanismus empfohlen).

[[edc-ti-2]]
=== TI-2: Datenbank-Interface (PostgreSQL, Multi-Schema)

*Description:*
Zugriff auf zwei separate PostgreSQL-Datenbanken: `workbench` (Entwicklung/Test) und `production` (freigegebene Templates). Cross-DB-Zugriff erfolgt über REST-APIs (Stufenübergabe), nicht über SQL. TI-2 Option A ist implementiert.

*Schema-Zuordnung (TI-2 implementiert):*

- blocpress-workbench → Datenbank `workbench` (E-1, E-11)
- blocpress-render → Datenbank `production` (E-1 vereinfacht: nur id, name, version, content, validFrom)
- _[geplant] blocpress-proof → Schema `proof` (E-1, E-3, E-4, E-5, E-6, E-7)_
- _[geplant] blocpress-admin → Schema `admin` (E-9, E-10)_

*Stufenübergabe-Mechanismus (TI-2 Option A):*

Cross-Datenbank-Zugriff erfolgt via REST-API, nicht via SQL:
`POST /api/render/templates/import` — blocpress-workbench sendet Template als Base64 an blocpress-render bei APPROVED-Status.
...
[[edc-e-1]]
=== E-1: Template / Baustein

*Description:*
Speichert Metadaten und Binärdaten von LibreOffice-Templates und Bausteinen. Das `type`-Feld unterscheidet zwischen Templates (druckbar) und Bausteinen (wiederverwendbare ODT-Fragmente, eingebunden via `text:section-source`). Beide Typen durchlaufen denselben Workflow (DRAFT → SUBMITTED → APPROVED). Schema: `workbench`. Freigegebene Templates werden bei APPROVED physisch in das Schema `production` kopiert (blocpress-render-Datenbank). Bausteine bleiben in `workbench` und werden zur Renderzeit via WebDAV-URL (`/api/webdav/released/bausteine/{name}.odt`) eingebunden. +
...
| 1 | id | UUID | Eindeutige ID (Primary Key, auto-generiert via gen_random_uuid())
| 2 | name | String(255) | Name (nicht eindeutig, da versioniert; UNIQUE auf name+valid_from+version)
| 3 | type | String(20) | Typ: "TEMPLATE" (Standard) oder "BAUSTEIN"
| 4 | version | Integer | Versionsnummer (1, 2, 3, ...; Auto-Versioning nach Name, UC-10.1)
| 5 | status | String(50) | Workflow-Status: "DRAFT", "SUBMITTED", "APPROVED", "REJECTED"
| 6 | valid_from | Timestamp | Aktivierungsdatum; wird auf now() gesetzt bei APPROVED-Transition
| 7 | created_at | Timestamp | Erstellungszeitpunkt (NOT NULL)
| 8 | content | bytea | ODT-Datei als Binärdaten (lazy-loaded)
| 9 | validation_result | JSONB | Validierungsergebnis: {errors, warnings, userFields, repetitionGroups, conditions}
|===

*Indizes:* `idx_template_name`, `idx_template_valid_from`, `idx_template_status`, `idx_template_type`

[[edc-e-2]]
=== E-2: Baustein

NOTE: E-2 ist nicht als separate Datenbanktabelle implementiert. Bausteine werden in der E-1-Tabelle (`template`) mit `type = 'BAUSTEIN'` gespeichert. Der Workflow (DRAFT → SUBMITTED → APPROVED) ist identisch mit Templates. Bausteine sind über den WebDAV-Endpoint unter `/api/webdav/bausteine/{name}.odt` (DRAFT) und `/api/webdav/released/bausteine/{name}.odt` (APPROVED) erreichbar.
...
| 1 | id | UUID | Eindeutige Generierungs-ID (Primary Key, auto-generiert)
...
| 1 | id | UUID | Eindeutige Prozess-ID (Primary Key, auto-generiert)
| 2 | template_id | UUID | Foreign Key zu E-1 (Template)
...
| 7 | freigeber_id | UUID | Foreign Key zu E-9 (Benutzer), nullable
...
| 1 | id | UUID | Eindeutige Review-ID (Primary Key, auto-generiert)
| 2 | template_id | UUID | Foreign Key zu E-1 (Template)
...
| 1 | id | UUID | Eindeutige Test-ID (Primary Key, auto-generiert)
| 2 | template_id | UUID | Foreign Key zu E-1 (Template)
...
| 1 | id | UUID | Eindeutige Pool-ID (Primary Key, auto-generiert)
...
| 1 | id | UUID | Eindeutige Benutzer-ID (Primary Key, auto-generiert)
...
| 1 | id | UUID | Eindeutige Konfigurations-ID (Primary Key, auto-generiert)
...
*Description:*
Speichert Test-Datensätze und optionale PDF-Baselines für Template-Designer zur lokalen Validierung. Schema: `workbench`. Separate von E-6 (Test Case in proof), da Designer ihre eigenen Testdaten verwalten und PDFs als "Expected Results" speichern können für zukünftige Regression-Tests. +
_Stories: UC-20 (Auto-generiertes Formular), UC-21 (Mehrere TestDataSets), TF-8 (PDF speichern), UC-11 (Regression-Vorbereitung)_
...
| 1 | id | UUID | Eindeutige TestDataSet-ID (Primary Key, auto-generiert)
...
| 4 | test_data | JSONB | Test-JSON-Daten (Struktur folgt template.validationResult.userFields)
...
| 8 | updated_at | Timestamp | Letzter Aktualisierungszeitpunkt, nullable
|===

*Relationships:*

- One-to-Many: Template (E-1) ← → TestDataSet (E-11)
...
*Description:*
Das System ist dimensioniert für < 5.000 Dokumente + Templates als Binärdaten in PostgreSQL.

[[edc-c-9]]
=== C-9: Docker-Container (SCS-Module)

*Description:*
Jedes SCS-Modul (blocpress-studio, blocpress-workbench, blocpress-proof, blocpress-render, blocpress-admin) wird als eigener Docker-Container betrieben. Eine gemeinsame `docker-compose.yml` orchestriert alle Container inkl. PostgreSQL und Elasticsearch. blocpress-render kann unabhängig skaliert werden.
