...
- `POST /api/template/{id}/render` - Dokument mit gespeichertem Template (ID) und JSON-Daten generieren (UC-10)
- Parameter: id (UUID path), requestBody (data: JSON, outputType: pdf|rtf|odt)
...
- `GET /api/workbench/templates/{id}` - Template-Binärdaten abrufen
- `GET /api/workbench/templates` - Template-Liste mit Status und Validierung abrufen
...
- `PUT /api/workbench/templates/{id}/status` - Template-Status ändern (DRAFT→SUBMITTED, etc., UC-5)
